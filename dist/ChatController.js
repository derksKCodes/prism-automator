...
export default class ChatController {
    ...
    async cancelPreviousUploads(signal) {
        if (signal?.aborted)
            throw new Error("Operation aborted");
    ...
    async askForEvaluation(promptContext, signal) {
        if (signal?.aborted)
            throw new Error("Operation aborted");
    ...
    async _waitForTableResponse(signal) {
        // 1. Wait for the new response container to appear
        const newResponse = await abortable(this.page.waitForSelector('model-response:not([data-old="true"])', { timeout: 60000 }), signal);
        if (!newResponse)
            throw new Error("New model-response did not appear!");
        ...
        await abortable(this.page.waitForFunction((res) => {
        ...
        }, { polling: 2000, timeout: 120000 }, newResponse), signal);
